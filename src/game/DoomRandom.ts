// Doom's deterministic random number generator — ported from m_random.c.
// Uses a fixed 256-byte lookup table, not Math.random().
// Critical for multiplayer determinism and matching original Doom behavior.
...
/** P_Random — returns 0–255, deterministic sequence */
...
/** M_Random — same table, separate index for non-gameplay use */
let mrndindex = 0;

export function M_Random(): number {

  mrndindex = ( mrndindex + 1 ) & 0xFF;
  return RNDTABLE[ mrndindex ];

}

/** Reset RNG indices (call on level start) */
