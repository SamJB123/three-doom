// Adapted browser subset; upstream attribution and changes in README.md.
function readString(view, offset, length = view.byteLength - offset) {
    let result = '';
    for (let i = 0; i < length; i++) { const byte = view.getUint8(offset + i); if (!byte) break; result += String.fromCharCode(byte); }
    return result;
}
