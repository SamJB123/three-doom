...
## 2026-09-13 — special lifecycle matrix and fast-door sound

**SPEC-01 / SPEC-02 / AUDIO-01.** Added full movement scenarios through `P_CrossSpecialLine` / `P_UseSpecialLine`, `T_VerticalDoor`, `T_PlatRaise`, `T_MoveFloor`, `T_MoveCeiling` and `EV_BuildStairs`: normal/fast doors, 30-second reopening doors, lifts, floor variants, cycling crushers with stop/resume, and slow/fast stairs. Assertions cover physical speeds/endpoints, 150/1050/105-tic waits, texture changes, trigger consumption, busy retrigger and sector release. [SPECIAL-COVERAGE.md](SPECIAL-COVERAGE.md) records the tested variants and remaining topology/obstruction/campaign scope.
...
## 2026-09-13 — diagonal wall contact, pickup shadow timing and bounded replay tooling

**PHYS-01 / PLAYER-01 / LOOP-02 / BUILD-01.** Refreshing the existing complete DEMO3 capture confirms the bullet side-probe change resolved the previous tic-2036 blood-height difference. The next mismatch at 2785 reduced to a single diagonal wall. `P_PointOnLineSide` now compares separately rounded FixedMul products as in the supplied C, rather than a floating determinant. This restores the original Y-only slide fallback at close wall contact. The synthetic regression reproduces the captured original position/momentum after one movement step.

After that fix, all captured fields agree through 3000 tics; the next captured difference is the player's shadow flag on the invisibility pickup tic. `P_GivePower` now applies MF_SHADOW immediately on both pickup paths, before later thinkers, instead of waiting for the next player tick. That correction has a behavioral regression; no subsequent full C agreement is claimed. Remaining later simulation differences are not closed. Paired local reference/port artifacts preserve the movement-fix comparison.

Diagnosed long replay shutdown independently: the branded macOS Chrome 152 Crashpad helper inherits the browser stderr socket and outlives Chrome, leaving Playwright's browser-close promise waiting on pipe closure. Inspector handles, lsof peer ownership and the Playwright teardown log confirmed the chain; stopping only the helper owned by the diagnostic run immediately completed teardown. Chrome crash-reporter/testing flags did not prevent the helper in this installed build, so no ineffective launch flag or blanket process cleanup was retained.

The same complete 3863-command replay passes repeatability/pause and exits cleanly using Playwright's bundled Chromium (31.9 seconds). Added `npm run test:replay -- [DEMO1 ... DEMO4]` to explicitly select that version-pinned diagnostic browser; a short invocation with PLAYWRIGHT_CHANNEL=chrome also correctly selects bundled Chromium and exits cleanly (4.6 seconds). Installed Chrome remains available for ordinary checks; its long-diagnostic helper limitation is documented, not described as fixed within Chrome itself.

Validation: typecheck, 169 unit tests and build pass. Long bundled-Chromium replay passed after the wall fix; the subsequent pickup correction has its focused unit check. Full release/campaign/device acceptance remains deferred. Next: remaining recorded DEMO1/DEMO2/DEMO4 simulation differences and a focused refresh after the pickup flag correction; no extension of trace lengths is required.

## 2026-09-13 — recorded differences closed; finite acceptance pass

**LOOP-02 / ACTOR-01 / COMBAT-01 / PHYS-01 / PLAYER-01.** All four complete supplied recordings now match the hosted original C in the captured field set: DEMO1 1710, DEMO2 2347, DEMO3 3863 and DEMO4 818 commands (8738 total). Browser repetitions also match with altered frame batching and pause. Paired traces and source/WAD/compiler provenance are retained locally; this closes the recorded-difference queue without extending recordings. It does not establish unrecorded actions, internal counters, DOS executable equivalence or complete campaign playthroughs.

Concrete corrections: `P_DamageMobj` consumes the pain roll before excluding zero-chance actors/charging skulls, and reproduces the occasional forward fall on a lethal hit from below. `R_PointOnSide`/`R_PointInSubsector` retain fractional positions and original boundary/rounding rules, including spawned effects/missiles; this corrects premature distant enemy wake-up and bullet puffs clipped by the wrong ceiling. Dynamic pickup meshes cannot award ammo a second time using stale spawn metadata. `P_ThingHeightClip` retains actor-before-line collision ordering, and dead players retain `P_XYMovement` corpse sliding over step edges.

**SPEC-01 / SPEC-02.** The finite obstruction/topology pass covers closing-door reversals versus close-only retry, obstructed lift return/recovery, repeated perpetual-platform cycles and stasis, and both plane/direction endpoint rollbacks. A busy staircase branch failed: `EV_BuildStairs` increments the next step height before checking specialdata. Correcting that order restores the original destination. These scenarios are source-derived, not new executed-C fixtures.

**BUILD-01 / BUILD-02.** Clean npm installation succeeded. Compatible lockfile updates resolve the twelve advisories reported by npm audit; a second clean install reports zero. Typecheck, 181 behavioral tests, production build, all-36-map WAD validation and four upstream audio PCM fixtures pass. Generated Cloudflare state is ignored. No second package-manager lockfile or IWAD/artifact was added.

Acceptance remains in progress: installed-Chrome/WebKit browser coverage, original-renderer scene comparisons and a bounded desktop performance baseline. The bundled headless Chromium WebGPU device is lost on this Mac; its default-renderer acceptance run fails three graphics/reload checks, while deterministic recordings complete. Installed Chrome renders the same scenes. Treat that as a documented diagnostic-browser graphics limitation, not a passing graphics gate. No exception filtering hides these failures.

Original `R_RenderPlayerView` frame capture now runs after the hosted recording, with separately hashed render/world harnesses. The frame comparison exposed a paused-demo heading reset in the port's control state; playback now synchronizes the stored control heading before menu isolation. A desktop/touch regression reproduces the prior 1.42-radian reset. Reference frames use base PLAYPAL; unticked HUD state is explicitly excluded from this reference scope.


## 2026-09-13 — playable-state README, desktop controls and acceptance checkpoint

**MENU-01 / SAVE-01 / BUILD-01 / LOOP-02.** Refreshed README around the playable four-episode game, supported npm installation, desktop/touch controls, browser saves and campaign presentation. Recorded the user's start-to-end playability feedback while retaining the explicit release matrix. Desktop layouts hide Map/Menu action buttons; Escape opens the menu and Tab toggles the automap. Touch layouts retain both buttons.

Four actual menu save/load checks at DEMO1/1000, DEMO2/1200, DEMO3/3050 and DEMO4/600 reproduce 160 continued simulation tics each, including world/player/weapon snapshots. The test driver advances explicit commands while the browser clock is paused; this checks restored encounters, not completed combat routes.

Validation: typecheck, 181 unit tests and build pass after the UI change. The full installed-Chrome suite passed 32 checks (4.6 minutes); nine scoped WebKit checks passed (45.3 seconds). Four subsequent Chrome checks cover desktop shortcuts/hidden buttons, touch layout/menu isolation and all four encounter saves (34.2 seconds). Final forced-WebGL bundled-Chromium recordings pass all four complete demos (2.1 minutes, clean exit); the refreshed C gate again matches all 8738 captured tics. All-36-map assets, four upstream audio fixtures and clean npm installation with zero audit advisories also pass.

Four original-renderer world-frame pairs were inspected. Their comparison found the paused heading bug fixed above; the supplied C's retail sky selection mixes mode/mission enums and is excluded from intended episode-sky evidence. HUD/palette timing remains outside that reference host. Bundled headless WebGPU device loss is documented; no browser errors were filtered to manufacture a pass.

Next release evidence: recorded campaign/secret-route saves, broader audiovisual listening and physical iOS/Android acceptance. User playability feedback is not replaced by a claim that the unrecorded routes are broken. Multiplayer and DOS executable/save-format compatibility remain separate.


**RENDER-01 acceptance finding.** The first 13-load performance run retained 57,099,516 extra JS-heap bytes after warm-up (roughly 6 MB per additional map load). `removeMobj` disposed actor geometry but omitted actor-owned material disposal. It now disposes the material exactly once without disposing shared textures prematurely; level teardown still disposes those textures. The new ownership/lifecycle regression passes, along with typecheck, all 182 unit tests and build. The initial software-WebGL run also missed frame/backlog budgets; it is preserved locally as a failed baseline, not counted as passing hardware performance.

Heap-retainer inspection identified a second root: Three's texture binding-group registry held old actor material/node graphs through the process-wide sprite colour atlas. `loadLevel` now disposes/recreates that shared atlas after old-level teardown, before creating new materials. Six-load diagnostic retention dropped to 654,936 bytes across the final two loads (previously 9,683,128 bytes over that same interval after material disposal alone). This uses public texture/material disposal, without patching renderer internals. A full native-desktop performance run follows with unchanged budgets.

The first native-GPU run measured p95 9.6 ms, p99 10.2 ms, 1,257,296 bytes retained heap growth and zero retained DOM-node growth. Its all-clock backlog reached 0.364 s, so that initial aggregate diagnostic failed. The measurement had included melt/page tics as gameplay; the runner now reports both the raw clock backlog and the backlog on frames that actually execute player/weapon simulation, using the same preset gameplay budget. Both results are preserved rather than retrospectively relabeling the first run. Installed Chrome also reproduced its known Crashpad teardown stall; the surviving helper was identified by its stderr socket to this runner and stopped, leaving unrelated browser processes untouched. The next native run uses bundled Chromium in visible mode.
