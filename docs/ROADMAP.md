...
| LOOP-02 | `P_RunThinkers`, `P_PlayerThink`, ticcmd ordering | partial: original C DEMO1 captured player/actor state 350-tic original C DEMO1 captured player/actor comparison passes | Match C action ordering, RNG consumption, input quantization and deterministic state traces |
...
| PHYS-01 | `p_map.c P_CheckPosition`, `P_TryMove`, `P_SlideMove` | partial: C slide projection, signed split rounding and step support checked | Solid actors, blocking flags, steps/dropoffs, diagonal sliding; player/monster/projectile consistency |
...
| ACTOR-02 | `P_NoiseAlert`, `A_Look`, `P_CheckSight` | partial: sight fixed-product regression and initial chase direction C-traced | Weapon noise reaches valid sectors, respects sound-block flags; ambush/sight behavior matches C |
| COMBAT-01 | `p_pspr.c`, `p_map.c` aiming/line attacks | partial: hit effects/RNG and player pain flag checked in C world trace | Correct melee range, autoaim, first-shot accuracy, spread, projectile wall impacts and BFG behavior |
