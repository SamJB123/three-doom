...
| LOOP-02 | `P_RunThinkers`, `P_PlayerThink`, ticcmd ordering | partial: DEMO1–3 350-tic C actor/target, inventory and sector comparisons pass; DEMO4 first diverges at tic 217 | Match C action ordering, RNG consumption, input quantization and deterministic state traces |
...
| COMBAT-01 | `p_pspr.c`, `p_map.c` aiming/line attacks | partial: hit effects/RNG, weapon input/state timing and per-shot bullet slope checked | Correct melee range, autoaim, first-shot accuracy, spread, projectile wall impacts and BFG behavior |
